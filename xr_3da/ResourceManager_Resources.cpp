...
public:
	HRESULT __stdcall Open(D3DXINCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData, UINT *pBytes)
	{
		...
		if (!R)			// possibly in shared directory or somewhere else - open directly	
		{
...
static BOOL	dcl_equal(D3DVERTEXELEMENT9 *a, D3DVERTEXELEMENT9 *b)
{
	...
	return 
		(0 == memcmp(a, b, a_size * sizeof(D3DVERTEXELEMENT9)));
}
...
CTexture *CResourceManager::_CreateTexture(LPCSTR _Name)
{
	...
	{
		CTexture *texture = xr_new<CTexture>();
		texture->dwFlags |= xr_resource_flagged::RF_REGISTERED;
		m_textures.insert(mk_pair(texture->set_name(Name), texture));
		texture->Preload();
