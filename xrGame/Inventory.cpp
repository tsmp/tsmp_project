...
void CInventory::Take(CGameObject *pObj, bool bNotActivate, bool strict_placement)
{
	...
	default:
	{
		...
		{
			if (!pIItem->RuckDefault() && CanPutInBelt(pIItem))
				result = Belt(pIItem);
			else
				result = Ruck(pIItem);
		}
...
bool CInventory::Ruck(PIItem pIItem)
{
	...
	bool in_slot = InSlot(pIItem); //вещь была в слоте
...
{
#pragma todo("tsmp: переписать без goto");
...
PIItem CInventory::Same(const PIItem pIItem, bool bSearchRuck) const  //ищем на поясе гранату такого же типа
{
...
PIItem CInventory::SameSlot(const u32 slot, PIItem pIItem, bool bSearchRuck) const   //ищем на поясе вещь для слота 
{
...
{
#pragma todo("tsmp: можно переписать, куча подобных функций");
...
PIItem CInventory::GetAny(const char *name) const  //search both (ruck and belt)
{
...
CInventoryItem	*CInventory::tpfGetObjectByIndex(int iIndex)
{
	...
	R_ASSERT(false); // сработает всегда
	return nullptr;  // 0 = at crash server !!
}
