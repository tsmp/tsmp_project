...
CActor::CActor() : CEntityAlive()
{
	...
	//-----------------------------------------------------------------------------------
	m_memory				= bIsDedicatedServer ? 0 : xr_new<CActorMemory>(this);
	m_bOutBorder			= false;
	hit_probability			= 1.f;
...
void	CActor::Hit							(SHit* pHDS)
{
	...
	if (!IsGameTypeSingle() && !bIsDedicatedServer)
	{
	...
	if(HDS.hit_type == ALife::eHitTypeWound || HDS.hit_type == ALife::eHitTypeStrike)
	{
		...
		hit_slowmo = 0.f;
	//---------------------------------------------------------------
	if (Level().CurrentViewEntity() == this && !bIsDedicatedServer && HDS.hit_type == ALife::eHitTypeFireWound)
	{
...
void CActor::shedule_Update(u32 DT)
{
	...
	if(this==Level().CurrentControlEntity() && !bIsDedicatedServer )
	{
