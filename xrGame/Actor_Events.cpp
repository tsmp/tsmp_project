		...
		case GE_OWNERSHIP_TAKE:
			{
				...
				{
					Msg("! WARNING: dead player [%d][%s] can't take items [%d][%s]", ID(), Name(), GO->ID(),
						GO->cNameSect().c_str());

					break;
				}

				if (inventory().CanTakeItem(smart_cast<CInventoryItem *>(GO)))
				{
					Obj->H_SetParent(smart_cast<CObject *>(this));
					inventory().Take(GO, false, true);
				...
				else
				{
					if (IsGameTypeSingle())
					{
						...
						Msg("! ERROR: Actor [%d][%s]  tries to drop on take [%d][%s]", ID(), Name(), GO->ID(),
							GO->cNameSect().c_str());
				}
			}
			break;
		...
		case GE_OWNERSHIP_REJECT:
			{
				...
				if (!Obj->getDestroy() && inventory().DropItem(GO))
				{
					...
					case GEG_PLAYER_ACTIVATEARTEFACT:
						{
							...
							if (!pArtefact)
							{
								Msg("! ERROR: GEG_PLAYER_ACTIVATEARTEFACT, Artefact not found. artefact_id = [%d]", id);
								break;
							}
