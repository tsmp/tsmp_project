...
{
	if (g_pGameLevel && !bIsDedicatedServer)
	{
...
void CGamePersistent::start_logo_intro		()
{
	...
	if (Device.dwPrecacheFrame==0)
	{
		...
		if (!bIsDedicatedServer && 0==xr_strlen(m_game_params.m_game_or_spawn) && NULL==g_pGameLevel)
		{
			VERIFY				(NULL==m_intro);
...
void CGamePersistent::OnFrame	()
{
	...
	++m_frame_counter;
#endif
	if (!bIsDedicatedServer && !m_intro_event.empty())	m_intro_event();
