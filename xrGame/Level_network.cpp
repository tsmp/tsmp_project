...
void CLevel::OnBuildVersionChallenge()
{
	...
	if (0 != strstr(Core.Params, "-nocheckres")) 
		auth = 1189233227;
	...
	if (!result)
	{
		...
		case 1:		//GameSpy CDKey
		{
			if (!xr_strcmp(ResultStr, "Invalid CD Key"))
				MainMenu()->SetErrorDialog(CMainMenu::ErrCDKeyInvalid);
			if (!xr_strcmp(ResultStr, "CD Key in use"))
				MainMenu()->SetErrorDialog(CMainMenu::ErrCDKeyInUse);
			if (!xr_strcmp(ResultStr, "Your CD Key is disabled. Contact customer service."))
				MainMenu()->SetErrorDialog(CMainMenu::ErrCDKeyDisabled);
			break;
		}
...
void CLevel::ClearAllObjects()
{
	...
	CObjectList &CObj = Level().Objects;

	while (ParentFound)
	{
		ProcessGameEvents();		

		u32 CLObjNum = CObj.o_count();			
		ParentFound = false;

		for (u32 i = 0; i < CLObjNum; i++)
		{
			CObject* pObj = CObj.o_get_by_iterator(i);				
